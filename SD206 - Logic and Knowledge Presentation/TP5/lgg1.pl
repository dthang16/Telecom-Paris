...
lgg(Shape1, Shape2, CommonShape, Cost) :-
	find_commonshape(Shape1, Shape2, CommonShape),
	ancestor(CommonShape, Shape1, Cost1),
	ancestor(CommonShape, Shape2, Cost2),
	Cost is max(Cost1, Cost2). 
	
find_commonshape(Shape, Shape, CommonShape) :-
	CommonShape = Shape.
find_commonshape(Shape1, Shape2, CommonShape) :-
	parent(Parent1, Shape1),
	parent(Parent2, Shape2),
	find_commonshape(Parent1, Parent2, CommonShape).*/
% Version du prof
...
form(pa).	% object is instanciated regardless of its specific properties
