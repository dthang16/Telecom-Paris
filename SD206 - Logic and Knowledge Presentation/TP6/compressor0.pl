...
increment(_, 0, Rest, Rest).

increment(X, Times, [Y|Result], Rest):-
    Y is X + 1,
    increment(Y, T, Result, Rest),
    Times is T + 1.
...
testCompress :-
    ...
    complexity(CL, CL1, _),
        write('Compressed 2 times version:\t'), write(CL1), nl,
    compress(L2, CL1),
    write('Decompress 1 time:\t'), write(L2), nl,
    compress(L1, L2),  % Funny: 'compress' is used to decompress
