...
parse(PhraseType, Phrases, FS):-
	...
	append(Chunk, Suff, Rest), % P = Pref + Chunk + Suff
	lexical_only(Suff),	% patch to avoid looping in some cases
	process_phrase(Head, Chunk),	% bottom up parsing using rule: Head --> <elements of Chunk>.
	append(Pref, [Head|Suff], NEWPhrases),  % Chunk is replaced by Head in P:  NEWP = Pref + Head + Suff
...
process_phrase(Head, Chunk) :-	
	% looking for Chunk as right-hand side of a rule.
	rule(Cat, RHS, HeadPosition),	% the rule reads:  Cat --> <elements of RHS> (RHS = right-hand side) 
	cats(Chunk, RHS), 	% keeps only syntactic categories in Chunk and matches with RHS
	% Cat has been recognized
		% writeln((Cat, RHS, Chunk)),
	merge(Chunk, HeadPosition, FS1),
	rescue(Cat, FS1, FS),
	Head =.. [Cat, FS].
