...
class Grammar:
	...
	def loadDCG(self, DCGFileName, LexicalRuleInputPattern=None, WordPattern=None, 
		...
		# np --> det, n.
		RuleTuples =  re.findall(	r'^([a-z]+)\s+' + ProductionSign + r'\s+([^\.\{]+)()\.',	# empty () for no pgm
									DCGTxt, re.M|re.S)
		...
		RuleTuples += re.findall(	r'^([a-z]+\([^\-]*\))\s+' + ProductionSign + r'\s+([^\.\{]+?)\{(.*?)\}\.', 
									DCGTxt, re.M|re.S)
		for R in RuleTuples:
			for RHSstr in R[1].split('|'):	# allowing for disjunctions = different RHS separated by '|'
	...
	def process(self, Sentence=None, Target=None):
		S = self.ask(Sentence)
		if S == '':	# second chance to get a phrase instead of a sentence
			S = self.ask(Prompt='or maybe a phrase > ')
			if S == '':	raise StopIteration
		elif Target is None:	Target = 's'
		self.parse(S, Target=Target)
		return S
