...
class Grammar(syn_Grammar.Grammar):
	...
	def match(self, W1, W2):
		...
		"""
		trace(f'unifying {W1}  and  {W2}', 7)
		# ------ W2 is supposed to be an element from the right-hand side of a rule
		if type(W1) == str:
	...
	def semantic_merge(self, Cat, Phrase1, Phrase2=None, HeadPosition=1):
		"""	Merging feature structures
		"""
		if Phrase2 is not None:	
			# trace(f'Merging {Phrase1} and {Phrase2} under {Cat}', 4)
			pass
		else:	trace(f'Reading {Phrase1} as {Cat}', 4)
		if HeadPosition ==2:
			# merging feature structures in reverse order (head last)
			return Phrase2.semantic_merge(Cat, Phrase1)
