...
add_edge(StartPos, EndPos, Cat, _, [], T, _) :-	% Extension
	...
	% add_edge(..., ..., ..., ..., ..., T2, 'Extended-right'), 
	add_edge(StartPos1, EndPos, Cat1, Done1, Rest, T2, 'Extended-right'),
	fail.	% forces backtracking
% Here we need yet another 'add_edge' clause to extend the active edge we are adding
% using an existing inactive edge (and, by doing so, leveraging memory to speed up parsing)
add_edge(StartPos, EndPos, Cat, Done, [Cat1|Rest], T, _) :-
	edge(EndPos, EndPos1, Cat1, _, [], T1, _),
	T =.. [H|L],
	append(L, [T1], L1),
	T2 =.. [H|L1],
	append(Done, [Cat1], Done2),
	add_edge(StartPos, EndPos1, Cat, Done2, Rest, T2, 'Extended-left'),
	fail.
